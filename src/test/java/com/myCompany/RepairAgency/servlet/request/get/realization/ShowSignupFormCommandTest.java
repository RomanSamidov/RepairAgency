...
class ShowSignupFormCommandTest {
    @Mock
    Path mockPath;
    @Mock
    ...
    void execute() {
        try (MockedStatic<PathFactory> ignored2 = Mockito.mockStatic(PathFactory.class);
            ...
            Mockito.when(PathFactory.getPath(Mockito.eq("path.page.forward.guest.signup"))).thenReturn(mockPath);

            assertEquals(mockPath, new ShowSignupFormCommand().execute(request, response));
            assertEquals("title.signup", request.getAttribute("title"));
    ...
    void allowedUserRoles() {
        assertEquals(Stream.of(Constants.ROLE.Guest).collect(Collectors.toList()),
                new ShowSignupFormCommand().allowedUserRoles());
    }
}